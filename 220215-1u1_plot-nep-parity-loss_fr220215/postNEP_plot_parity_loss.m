...
r_main_path='/home/wyz/work-master/proj_C-related/220215_2024_Wang_C_nep-train';   %??????
job_index=[1];  %????

for i=job_index
    index=1;
    read_dire=[r_main_path, '/job_',num2str(i)];    % ??????
    ...
    if isfile([read_dire,'/energy_train.out'])
        ...
        disp('energy_train.out is missing ...');
    end

    if isfile([read_dire,'/force_train.out'])
        ...
        disp('force_train.out is missing ...');
    end

    if isfile([read_dire,'/virial_train.out'])
        virial = load([read_dire,'/virial_train.out']);
        figure;
        %figure('Position',[400,250,500,500]);
        %axes('position',[0.17,0.165,0.81,0.81]);
        virial_nep=virial(1:39708,1);
        virial_dft=virial(1:39708,2);
        %virial_nep=reshape(virial(:,1:6),length(virial)*6,1);
        %virial_dft=reshape(virial(:,7:12),length(virial)*6,1);
        %mask= virial_dft ~= -1000000;  % NEP marks the conf without DFT virials with -1000000
        %virial_nep=virial_nep(mask);
        %virial_dft=virial_dft(mask);
        ...
        print([write_dire,'/',fn],'-dpng','-r200');
        close;
        train_virial=[virial_dft, virial_nep];
    else
        disp('virial_train.out is missing ...');
    end

    if isfile([read_dire,'/energy_test.out'])
        ...
        disp('energy_test.out is missing ...');
    end

    if isfile([read_dire,'/force_test.out'])
        ...
        disp('force_test.out is missing ...');
    end

    if isfile([read_dire,'/virial_test.out'])
        virial = load([read_dire,'/virial_test.out']);
        figure;
        %figure('Position',[400,250,500,500]);
        %axes('position',[0.17,0.165,0.81,0.81]);
        virial_nep=virial(1:end,1);
        virial_dft=virial(1:end,2);
        %virial_nep=reshape(virial(:,1:6),length(virial)*6,1);
        %virial_dft=reshape(virial(:,7:12),length(virial)*6,1);
        %mask= virial_dft ~= -1000000;  % NEP marks the conf without DFT virials with -1000000
        %virial_nep=virial_nep(mask);
        ...
        print([write_dire,'/',fn],'-dpng','-r200');
        close;
        test_virial=[virial_dft, virial_nep];
    else
        disp('virial_test.out is missing ...');
    end
end
